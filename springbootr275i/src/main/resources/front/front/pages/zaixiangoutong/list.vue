...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view :style='{"padding":"0","alignItems":"flex-start","flexWrap":"wrap","background":"url() fixed,#fff","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
			...
			<view class="uni-product-list" :style='{"padding":"0px 24rpx 24rpx","margin":"24rpx 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"calc(100% - 168rpx)","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"padding":"0px","boxShadow":"0 8rpx 0px #87acf1","margin":"0 0 40rpx","borderColor":"#e9be70","display":"flex","borderRadius":"0px","flexWrap":"wrap","borderWidth":"0px 0px 0px","background":"rgba(255,255,255,1)","width":"46%","position":"relative","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
					...
					<view class="uni-product-title" :style='{"padding":"4rpx 20rpx","margin":"0 0 8rpx","whiteSpace":"nowrap","overflow":"hidden","color":"#333","borderRadius":"20%","textAlign":"center","background":"none","width":"96%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>{{product.zixunshijian}}</view>
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto","order":"5"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('zaixiangoutong','修改')) || (!userid && isAuthFront('zaixiangoutong','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('zaixiangoutong','删除')) || (!userid && isAuthFront('zaixiangoutong','删除'))" @click.stop="onDeleteTap(product.id)">
		...
		<button :style='{"border":"2rpx solid #fff","cursor":"pointer","boxShadow":"0 4rpx 8rpx rgba(0,0,0,.3)","color":"#fff","textAlign":"center","bottom":"160rpx","right":"128rpx","outline":"none","borderRadius":"100%","background":"#6195ee","width":"88rpx","lineHeight":"88rpx","fontSize":"28rpx","position":"fixed","height":"88rpx","zIndex":"11111"}' v-if="userid && isAuth('zaixiangoutong','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"2rpx solid #fff","cursor":"pointer","boxShadow":"0 4rpx 8rpx rgba(0,0,0,.3)","color":"#fff","textAlign":"center","bottom":"160rpx","right":"128rpx","outline":"none","borderRadius":"100%","background":"#6195ee","width":"88rpx","lineHeight":"88rpx","fontSize":"28rpx","position":"fixed","height":"88rpx","zIndex":"11111"}' v-if="!userid && isAuthFront('zaixiangoutong','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
...
<script>
	export default {
        ...
        params['order'] = 'desc';
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`zaixiangoutong`, params);
                } else {
                    res = await this.$api.list(`zaixiangoutong`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('zaixiangoutong', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`zaixiangoutong`, searchForm);
                } else {
                    res = await this.$api.list(`zaixiangoutong`, searchForm);
                }
				// 如果是第一页数据置空

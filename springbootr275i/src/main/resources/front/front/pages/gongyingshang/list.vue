...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
			...
			<view class="cu-bar bg-white search" style="width:100%" :style="[{top:CustomBar + 'px'}]">
				<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
					...
					<view v-if="queryIndex==0" class="search-form round">
						...
						<input v-model="searchForm.gongyingzhanghao" type="text" placeholder="供应账号" ></input>
					</view>
				<view class="action">
			...
			<view class="uni-product-list" :style='{"padding":"0px 24rpx 24rpx","margin":"24rpx 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"calc(100% - 168rpx)","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"padding":"0px","boxShadow":"0 8rpx 0px #87acf1","margin":"0 0 40rpx","borderColor":"#e9be70","display":"flex","borderRadius":"0px","flexWrap":"wrap","borderWidth":"0px 0px 0px","background":"rgba(255,255,255,1)","width":"46%","position":"relative","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto","order":"5"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('gongyingshang','修改')) || (!userid && isAuthFront('gongyingshang','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('gongyingshang','删除')) || (!userid && isAuthFront('gongyingshang','删除'))" @click.stop="onDeleteTap(product.id)">
		...
		<button :style='{"border":"2rpx solid #fff","cursor":"pointer","boxShadow":"0 4rpx 8rpx rgba(0,0,0,.3)","color":"#fff","textAlign":"center","bottom":"160rpx","right":"128rpx","outline":"none","borderRadius":"100%","background":"#6195ee","width":"88rpx","lineHeight":"88rpx","fontSize":"28rpx","position":"fixed","height":"88rpx","zIndex":"11111"}' v-if="userid && isAuth('gongyingshang','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"2rpx solid #fff","cursor":"pointer","boxShadow":"0 4rpx 8rpx rgba(0,0,0,.3)","color":"#fff","textAlign":"center","bottom":"160rpx","right":"128rpx","outline":"none","borderRadius":"100%","background":"#6195ee","width":"88rpx","lineHeight":"88rpx","fontSize":"28rpx","position":"fixed","height":"88rpx","zIndex":"11111"}' v-if="!userid && isAuthFront('gongyingshang','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.gongyingzhanghao="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.gongyingzhanghao){
					params['gongyingzhanghao'] = '%' + this.searchForm.gongyingzhanghao + '%'
				}
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`gongyingshang`, params);
                } else {
                    res = await this.$api.list(`gongyingshang`, params);
                }
				// 如果是第一页数据置空
			...
			async search(){
				...
				if(this.searchForm.gongyingzhanghao){
					searchForm['gongyingzhanghao'] = '%' + this.searchForm.gongyingzhanghao + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`gongyingshang`, searchForm);
                } else {
                    res = await this.$api.list(`gongyingshang`, searchForm);
                }
				// 如果是第一页数据置空
